...
def chartemp():
	...
	try:
		...
		if data:
			...
			plt.ylabel("Salaries")
			plt.title("Top highest salaryed Employee")
		
			chart = Toplevel(root)
			chart.title("Employee salaries chart")
			chart.geometry("800x600+400+100")
			chart.configure(bg="lightsteelblue")
			canvas = FigureCanvasTkAgg(plt.gcf(),master=chart)
			canvas.draw()
